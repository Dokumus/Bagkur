...
if (!QUICK) {
  console.log('\n3. Dashboard build');
  const hasGo = run('go version') !== null;
  if (!hasGo) {
    warn('Dashboard build skipped (go is not installed)');
  } else {
    const outPath = process.platform === 'win32' ? 'NUL' : '/tmp/career-dashboard-test';
...
for (const pattern of leakPatterns) {
  const result = run(
    `git grep -n "${pattern}" -- "*.md" "*.yml" "*.html" "*.mjs" "*.sh" "*.go" "*.json" ${devNull}`
  );
  if (result) {
...
const absPathResult = run(
  `git grep -n "/Users/" -- "*.mjs" "*.sh" "*.md" "*.go" "*.yml" ${devNull}`
);
if (!absPathResult) {
