...
## Cadencia y archivado (config del usuario)

- **Frecuencia automática:** cada **24 horas** (configurado vía `/schedule`). El usuario quiere un scan diario para no perder ofertas ni deadlines.
- **Archivado de ofertas no revisadas:** el dashboard web (`web-dashboard/`) trata como "stale" toda oferta aún en estado `evaluated` que esté **vencida (deadline pasado)** o tenga **más de 30 días** sin acción, y la mueve a la pestaña **Arşiv** (fuera de la lista activa "Başvurulacak"). Siguen en `applications.md`, así que el dedup las sigue bloqueando: NO reaparecen en futuros scans. El usuario puede revisarlas en Arşiv cuando quiera.
...
Leer también `config/locations.json` (gestionado desde el dashboard web → panel "Lokasyonlar"):
- `regions`: lista de regiones objetivo. Cada una con `country`, `cities`, `keywords`, `enabled`.
- `remote_ok`: si `true`, las ofertas remote también pasan el filtro de ubicación.
- **Este archivo es la fuente de verdad del alcance geográfico.** El usuario añade/quita países y ciudades desde el dashboard, y eso alimenta directamente este scan. Configura tu propio alcance (países/ciudades objetivo) ahí.
...
6.5. **Filtrar por ubicación** usando `config/locations.json`:
   Para cada oferta, determinar su ubicación (del título, snippet, o JD: ciudad/país/"Remote").
   Una oferta **pasa** el filtro si cumple AL MENOS UNA:
   - Su país coincide con el `country` de alguna región con `enabled: true`
   - Su ciudad coincide con alguna `cities` de una región habilitada
   - Su texto contiene alguna `keywords` de una región habilitada (case-insensitive)
   - Es **Remote** y `remote_ok: true` (verificar que el remote sea compatible con la zona horaria del candidato; remote global o EU/CET pasa, remote restringido a US-only no)
   Las ofertas que NO pasan → registrar en `scan-history.tsv` con status `skipped_location` y descartar.
   Si la ubicación es ambigua/desconocida, NO descartar: pasar a evaluación y anotar la incertidumbre.
...
10. **Ofertas filtradas por ubicación**: registrar con status `skipped_location`
...
Filtradas por ubicación: N (fuera del alcance geográfico)
