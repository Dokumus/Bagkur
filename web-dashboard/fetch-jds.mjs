...
async function main() {
  // Optional --pipeline <path> lets the freelance lane reuse this fetcher against
  // data/freelance-pipeline.md instead of the default full-time pipeline.
  const argIdx = process.argv.indexOf("--pipeline");
  const pipelineRel = argIdx >= 0 && process.argv[argIdx + 1] ? process.argv[argIdx + 1] : join("data", "pipeline.md");
  const pipelinePath = resolve(ROOT, pipelineRel);
  const md = await readFile(pipelinePath, "utf8");
  // Anchor to a real heading line so a stray "## …" inside an offer cell can't truncate.
