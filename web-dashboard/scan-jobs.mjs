...
// --dry-run: kaynakları çeker, filtreleri uygular, ama pipeline.md/scan-history'ye
// hiçbir şey yazmaz. Yeni kaynak eklerken kapsamı görmek için.
...
async function discoverAts(pageUrl) {
  ...
  m = h.match(/apply\.workable\.com\/([a-z0-9_-]+)/i);
  if (m) return `https://apply.workable.com/${m[1]}/`;
  return null;
}
...
// ---- filters ----
// Türkçe başlıklar için harf katlama: "İş Zekası" / "IŞ ZEKASI" / "Is Zekasi"
// hepsi aynı forma insin. JS'in toLowerCase()'i "İ" → "i̇" (birleşik nokta) ürettiği
// ve "I" → "ı" ayrımını bilmediği için düz toLowerCase eşleşmeyi kaçırıyordu.
function foldTr(s) {
  return (s || "")
...
function titleOk(title, positive, negative) {
  ...
  if (negative.some((n) => t.includes(foldTr(n)))) return false;
  return positive.some((p) => t.includes(foldTr(p)));
}
...
async function fromSmartRecruiters(slug, name) {
  const out = [];
  // Büyük panolarda (ör. Delivery Hero: 1000+ ilan, bunun 10'u Türkiye) ilk 200
  // kayıt hedef ülkeyi hiç içermeyebiliyordu — daha derin sayfalama gerekiyor.
...
// Workable: public job-board API. TR sirketlerinin cogu Workable kullaniyor ama
// tarayicida hic desteklenmiyordu - bu panolardaki ilanlar hic gorunmuyordu.
async function fromWorkable(slug, name) {
  const data = await getJSON(`https://apply.workable.com/api/v2/accounts/${slug}/jobs`, {
    method: "POST",
  ...
  return data.results.map((j) => {
    const l = j.location || {};
    const loc = j.remote ? "Remote" : [l.city, l.region, l.country].filter(Boolean).join(", ");
    return { title: j.title, company: name, url: `https://apply.workable.com/${slug}/j/${j.shortcode}/`, location: loc };
  });
}
...
function atsFor(c) {
  ...
  if (u.includes("workable.com")) { const slug = (u.match(/workable\.com\/(?:api\/v\d\/(?:widget\/)?accounts\/)?([^/?]+)/) || [])[1]; return { kind: "workable", slug, fetch: () => fromWorkable(slug, c.name) }; }
...
async function loadDedup() {
  ...
  // Purged jobs (prune-tracker.mjs). Their tracker rows + reports are deleted to
  // keep the repo lean, so this tombstone file is the ONLY thing stopping the
  // scanner from re-discovering and re-evaluating them every single day.
  // Feeds BOTH dedup sets: url and company+role.
  try {
    ...
    for (const line of rej.split("\n")) {
      ...
      if (url && url.startsWith("http")) seenUrls.add(url.split("?")[0]);
...
async function main() {
  ...
  { const seen = new Set();
    for (const r of (locations.regions || []).filter((r) => r.enabled !== false)) {
      // `li_locations` (optional) wins: search each listed city/province separately
      // instead of collapsing to the country. Use it when a region spans more than
      // two cities but you still want per-city coverage (e.g. Türkiye's 7 provinces).
      const explicit = Array.isArray(r.li_locations) ? r.li_locations.filter(Boolean) : null;
      const derived = [(r.cities || []).length && r.cities.length <= 2 ? `${r.cities[0]}, ${r.country || r.label}` : (r.country || r.label)];
      for (const s of (explicit && explicit.length ? explicit : derived)) {
  ...
  // Kendi kariyer portalını kullanan şirketler (TR'de çoğunluk: Garanti BBVA
  // Teknoloji, İş Bankası, THY…) atsFor() ile eşleşmiyor ve hiç çekilmiyordu.
  // Sayfayı bir kez açıp gömülü ATS panosunu ara; sonucu 7 gün önbelleğe al ki
  // her taramada aynı sayfalar yeniden indirilmesin.
  {
    const CACHE_PATH = join(ROOT, "config", "ats-discovery.json");
    const TTL_MS = 7 * 24 * 3600e3;
    let cache = {};
    try { cache = JSON.parse(await readFile(CACHE_PATH, "utf8")); } catch {}
    let discovered = 0, probed = 0;
    for (const c of companies) {
      if (atsFor(c) || !c.careers) continue;                 // zaten ATS ya da URL yok
      const hit = cache[c.careers];
      const fresh = hit && Date.now() - Date.parse(hit.at || 0) < TTL_MS;
      let found = fresh ? hit.ats : null;
      if (!fresh) {
        probed++;
        found = await discoverAts(c.careers);
        cache[c.careers] = { ats: found, at: new Date().toISOString() };
      }
      if (found) { c.api = found; discovered++; }
    }
    try { await writeFile(CACHE_PATH, JSON.stringify(cache, null, 1), "utf8"); } catch {}
    stats.discovery = { probed, discovered };
  }
  ...
  // ---- iş panoları (config/job-boards.json): kariyer.net, techcareer.net,
  // Arbeitnow, Remotive, Jobicy, Himalayas. Şirket ATS'i olmayan ilanları da yakalar.
  try {
    const boards = await loadBoards();
    const { rows, stats: bstats } = await fetchBoards(boards);
    candidates.push(...rows);
    stats.boards = bstats;
  } catch (e) {
    stats.boards = [{ id: "job-boards", count: 0, error: e.message }];
  }
  ...
  // write
  if (newOffers.length && !DRY_RUN) {
    const pipePath = join(ROOT, "data", "pipeline.md");
    let pipe = await readFile(pipePath, "utf8");
    // Sanitize cells: "|" is the delimiter and "#" starts a heading — a stray one
    // inside a scraped title/company/location would corrupt the pipeline structure.
  ...
  if (stats.discovery) console.log(`Kendi portalında gömülü ATS keşfi: ${stats.discovery.discovered} pano bulundu (${stats.discovery.probed} sayfa denendi, gerisi önbellekten)`);
  console.log(`LinkedIn guest cards: ${stats.linkedin}`);
  if (stats.boards?.length) {
    console.log(`İş panoları: ${stats.boards.map((b) => `${b.id}(${b.count})`).join(", ")}`);
    // Kırılan kaynak sessizce kaybolmasın: 0 dönen adaptör burada görünür.
    const broken = stats.boards.filter((b) => b.error);
    if (broken.length) console.log(`!! Pano sorunlu: ${broken.map((b) => `${b.id} — ${b.error}`).join(" | ")}`);
  }
  console.log(`Candidates: ${stats.found} | skipped title: ${stats.title} | skipped location: ${stats.location} | dedup: ${stats.dup}`);
  console.log(`>>> NEW in-scope ${DRY_RUN ? "(yazılmadı)" : "added to pipeline"}: ${DRY_RUN ? newOffers.length : stats.added}`);
  const byLoc = {};
